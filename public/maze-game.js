...
function startMazeGame() {
    ...
    initMazeUI();
    loadLevel(0); // Начинаем с первого уровня
}
...
// Функция для добавления стен вокруг клетки (исправленная)
...
function renderMaze() {
    ...
    console.log('gameState.maze размер:', gameState.maze.length);
    
    if (!mazeGrid) {
        console.error('mazeGrid не найден!');
        mazeGrid = document.getElementById('maze-grid');
        if (!mazeGrid) {
            console.error('Элемент maze-grid не существует в DOM');
            return;
        }
    }
    ...
    for (let y = 0; y < size; y++) {
        for (let x = 0; x < size; x++) {
            ...
            cell.dataset.x = x;
            cell.dataset.y = y;
            cell.style.width = `${MAZE_CONFIG.cellSize}px`;
            cell.style.height = `${MAZE_CONFIG.cellSize}px`;
            ...
            if (gameState.maze[y][x] === 1) {
                ...
                cell.style.backgroundColor = MAZE_CONFIG.wallColor;
                cell.style.color = '#9ca3af';
            } 
            // Путь
            else {
                cell.classList.add('path');
                cell.style.backgroundColor = MAZE_CONFIG.pathColor;
...
function checkStarCollection() {
    ...
    if (starIndex !== -1) {
        ...
        const starCell = document.querySelector(`.maze-cell[data-x="${gameState.player.x}"][data-y="${gameState.player.y}"]`);
        if (starCell) {
...
function showHint() {
    ...
    if (path.length > 0) {
        ...
        path.forEach((cell, index) => {
            setTimeout(() => {
                const cellElement = document.querySelector(`.maze-cell[data-x="${cell.x}"][data-y="${cell.y}"]`);
                if (cellElement) {
...
function updateUI() {
    if (!movesCount || !timerCount || !starsCount || !levelInfo) {
        console.warn('Не все UI элементы найдены');
        return;
    }
    ...
    if (levelInfo) {
        ...
        const levelNameEl = levelInfo.querySelector('.level-name');
        if (levelNameEl) {
            levelNameEl.textContent = `Уровень ${level.id}: ${level.name}`;
        }
    }
    
    // Обновляем статистику
    if (movesCount) movesCount.textContent = gameState.moves;
    if (starsCount) starsCount.textContent = `${gameState.collectedStars}/${level.stars}`;
...
if (typeof window !== 'undefined') {
    ...
    window.showMazeMode = function() {
        // Эта функция должна быть в app.js
